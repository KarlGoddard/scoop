...
function createComment(url, request) {
  const newId = database.nextCommentId;
  const requestComment = request.body && request.body.comment;
  const response = {};

  if (requestComment && requestComment.comment && requestComment.username
    && requestComment.articleId) {
        const newComment = {
          'id': newId,
          'body': requestComment.comment,
    ...
    database.users[newComment.username].articleIds.push(newComment.id);
...
function deleteComment(url, request) {
  ...
  if (savedComment) {
    ...
    const userCommentIds = database.users[savedComment.username].articleIds;
    userCommentIds.splice(userCommentIds.indexOf(id), 1);
    response.status = 204;
  } else {
    response.status = 400;
